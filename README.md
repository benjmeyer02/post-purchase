...
Before running local development for the first time, configure your environment variables and database connection. You can copy `.env.example` to `.env` and update the values for your local Postgres instance and Shopify app credentials. If you need to create or update database tables, run:

```shell
npm run db:setup
```

After the database is already set up, `npm run dev` only runs `prisma generate` and won't re-run migrations on every start.
